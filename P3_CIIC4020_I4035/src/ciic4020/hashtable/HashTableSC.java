...
public class HashTableSC<K, V> implements Map<K, V> {
	...
	/*	This method doubles the amount of buckets whenever the load factor of the 
	 * 	hash table exceeds the default load factor. 
	 */
	@SuppressWarnings("unchecked")
	private void reHash() {
		/* First, create new array of buckets to contain twice the elements of the original buckets array. */
		List<BucketNode<K, V>>[] newBuckets = new LinkedList[buckets.length * 2];
		/* Append an empty linked list of bucketNodes to the array. */
		for(int i = 0; i < newBuckets.length; i++) {
			newBuckets[i] = new LinkedList<BucketNode<K, V>>();
		}
		for(int i = 0; i < buckets.length; i++) {
			for(BucketNode<K, V> BN: buckets[i]) {
				/* Find the new target bucket where the element belongs in the new bucket list. */
				int targetBucket = hashFunction.hashCode(BN.getKey()) % newBuckets.length;
